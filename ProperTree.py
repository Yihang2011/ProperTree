...
class ProperTree:
    def __init__(self, plists = []):
        ...
        self.expand_check = tk.Checkbutton(self.settings_window, text = "Expand Children When Opening Plist",
                                           variable = self.expand_on_open, command = self.expand_command)
        self.xcode_check = tk.Checkbutton(self.settings_window,
                                          text = "Use Xcode-Style <data> Tags (Inline) in XML Plists",
                                          variable = self.use_xcode_data, command = self.xcode_command)
        self.sort_check = tk.Checkbutton(self.settings_window, text = "Ignore Dictionary Key Order",
                                         variable = self.sort_dict_keys, command = self.sort_command)
        self.ignore_case_check = tk.Checkbutton(self.settings_window, text = "Ignore Case When Stripping Comments",
                                                variable = self.comment_ignore_case, command = self.ignore_case_command)
        self.check_string_check = tk.Checkbutton(self.settings_window,
                                                 text = "Check String Values When Stripping Comments",
                                                 variable = self.comment_check_string,
        ...
        self.plist_type_menu = tk.OptionMenu(self.settings_window, self.plist_type_string, "XML", "Binary",
        ...
        self.data_type_menu = tk.OptionMenu(self.settings_window, self.data_type_string, "Hex", "Base64",
                                            command = self.change_data_type)
        ...
        self.int_type_menu = tk.OptionMenu(self.settings_window, self.int_type_string, "Decimal", "Hex",
                                           command = self.change_int_type)
        ...
        self.bool_type_menu = tk.OptionMenu(self.settings_window, self.bool_type_string, "True/False", "YES/NO",
                                            "On/Off", "1/0", u"\u2714/\u274c", command = self.change_bool_type)
        ...
        self.snapshot_menu = tk.OptionMenu(self.settings_window, self.snapshot_string, "Auto-detect",
                                           command = self.change_snapshot_version)
        ...
        self.schema_check = tk.Checkbutton(self.settings_window, text = "Force Update Snapshot Schema",
                                           variable = self.force_schema, command = self.schema_command)
        ...
        self.op_scale = tk.Scale(self.settings_window, from_ = 25, to = 100, orient = tk.HORIZONTAL,
        ...
        self.hl_canvas = tk.Canvas(self.settings_window, height = 20, width = 30, background = "black",
                                   relief = "groove", bd = 2)
        ...
        self.r1_canvas = tk.Canvas(self.settings_window, height = 20, width = 30, background = "black",
                                   relief = "groove", bd = 2)
        ...
        self.r2_canvas = tk.Canvas(self.settings_window, height = 20, width = 30, background = "black",
                                   relief = "groove", bd = 2)
        ...
        self.bg_canvas = tk.Canvas(self.settings_window, height = 20, width = 30, background = "black",
                                   relief = "groove", bd = 2)
        self.bg_canvas.grid(row = 5, column = 4, sticky = "we", padx = 10)
        self.ig_bg_check = tk.IntVar()
        self.ig_bg = tk.Checkbutton(self.settings_window, text = "Header Text Ignores BG Color",
                                    variable = self.ig_bg_check, command = self.check_ig_bg_command)
        self.ig_bg.grid(row = 6, column = 3, sticky = "w", padx = 10)
        self.bg_inv_check = tk.IntVar()
        self.bg_inv = tk.Checkbutton(self.settings_window, text = "Invert Header Text Color",
                                     variable = self.bg_inv_check, command = self.check_bg_invert_command)
        ...
        self.font_check = tk.Checkbutton(self.settings_window, text = "Use Custom Font Size",
                                         variable = self.custom_font, command = self.font_command)
        ...
        self.font_custom_check = tk.Checkbutton(self.settings_window, text = "Use Custom Font",
                                                variable = self.font_var, command = self.font_select)
        self.font_custom = ttk.Combobox(self.settings_window, state = "readonly", textvariable = self.font_family,
        ...
        self.update_check = tk.Checkbutton(self.settings_window, text = "Check For Updates At Start",
                                           variable = self.update_int, command = self.update_command)
        self.update_check.grid(row = 16, column = 0, sticky = "w", padx = 10, pady = (5, 0))
        self.notify_once_int = tk.IntVar()
        self.notify_once_check = tk.Checkbutton(self.settings_window, text = "Only Notify Once Per Version",
                                                variable = self.notify_once_int, command = self.notify_once)
        self.notify_once_check.grid(row = 17, column = 0, sticky = "w", padx = 10, pady = (0, 10))
        self.update_button = tk.Button(self.settings_window, text = "Check Now",
        ...
        f_option = tk.OptionMenu(self.tk, self.f_title, "Ascii", "Base64", "Decimal", "Hex",
                                 command = self.change_from_type)
        t_option = tk.OptionMenu(self.tk, self.t_title, "Ascii", "Base64", "Decimal", "Hex",
        ...
        self.settings_window.protocol("WM_DELETE_WINDOW",
                                      lambda x = self.settings_window: self.close_window(window = x))
        ...
        if os.path.exists("Scripts/version.json"):
            try:
                self.version = json.load(open("Scripts/version.json"))
            except:
                pass
        os.chdir(cwd)
    ...
    def check_for_updates(self, user_initiated = False):
        if self.is_checking_for_updates:  # Already checking
            if user_initiated:
                ...
                self.tk.bell()
                mb.showerror("Already Checking For Updates",
                             "An update check is already in progress.  If you consistently get this error when "
                             "manually checking for updates - it may indicate a netowrk issue.")
            return
    ...
    def get_latest_tex(self):
        ...
        # We'll leverage multiprocessing to avoid UI locks if the update checks take too long
        p = multiprocessing.Process(target = _update_tex,
                                    args = (self.tex_queue, self.tex_url, self.get_best_tex_path()))
        p.daemon = True
        p.start()
    ...
    def set_window_opacity(self, opacity = None, window = None):
        if opacity is None:
            try:
                opacity = min(100, max(int(self.settings.get("opacity", 100)), 25))
            except:
                opacity = 100  # failsafe
    ...
    def swap_colors(self, color_type = None):
        ...
        # Find out if we're setting it to light or dark mode - and if on macOS/Windows + using the system's current
        # settings,
    ...
    def update_canvases(self):
        ...
        color_1 = "".join([x for x in self.settings.get("alternating_color_1", default_color["alternating_color_1"]) if
                           x.lower() in "0123456789abcdef"])
        color_2 = "".join([x for x in self.settings.get("alternating_color_2", default_color["alternating_color_2"]) if
                           x.lower() in "0123456789abcdef"])
        color_h = "".join([x for x in self.settings.get("highlight_color", default_color["highlight_color"]) if
                           x.lower() in "0123456789abcdef"])
        color_b = "".join([x for x in self.settings.get("background_color", default_color["background_color"]) if
                           x.lower() in "0123456789abcdef"])
    ...
    def update_settings(self):
        ...
        snapshot_versions = [
            "{} -> {}".format(x["min_version"], x.get("max_version", "Current")) if x["min_version"] != x.get(
                "max_version", "Current") else x["min_version"] for x in self.snapshot_data if
            "min_version" in x and len(x["min_version"])]
        snapshot_choices = ["Auto-detect", "Latest"] + sorted(snapshot_versions, reverse = True)
        for choice in snapshot_choices:
            self.snapshot_menu["menu"].add_command(label = choice, command = tk._setit(self.snapshot_string, choice,
                                                                                       self.change_snapshot_version))
    ...
    def update_recents(self):
        ...
        targets = [self] if str(sys.platform) == "darwin" else [w for w in self.stackorder(self.tk) if
                                                                not w in self.default_windows]
    ...
    def open_recent(self, path = None):
        ...
        if not (os.path.exists(path) and os.path.isfile(path)):
            self.tk.bell()
            mb.showerror("An Error Occurred While Opening {}".format(os.path.basename(path)),
    ...
    def open_plist_from_app(self, *args):
        ...
        try:
            ...
            for arg in args:
                ...
                existing_window = next((window for window in windows if
                                        not window in self.default_windows and window.current_plist == arg), None)
                if existing_window:
    ...
    def open_plist(self, event = None):
        ...
        path = fd.askopenfilename(title = "Select plist file",
                                  filetypes = file_types)  # ,parent=current_window) # Apparently parent here breaks
        # on 10.15?
    ...
    def pre_open_with_path(self, path, current_window = None):
        ...
        path = os.path.abspath(os.path.expanduser(path))
        windows = self.stackorder(self.tk)
        if current_window == None and len(windows) == 1 and windows[0] == self.start_window and windows[
            0].edited == False and windows[0].current_plist == None:
            # Fresh window - replace the contents
            current_window = windows[0]
        # Verify that no other window has that file selected already
    ...
    def quit(self, event_or_signum = None, frame = None):
        ...
        if len(unsaved) > 1:  # Ask for review
            answer = mb.askyesnocancel(
                ...
                "all unsaved changes will be lost)".format(
                    len(unsaved),
